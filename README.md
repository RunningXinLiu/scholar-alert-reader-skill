...
For a cleaner research knowledge graph, use a local taxonomy and `--graph-mode research`:
...
python3 scripts/scholar_reader.py obsidian-graph \
  ...
  --kb-dir knowledge_base \
  --graph-mode research \
  --taxonomy profiles/knowledge_graph_taxonomy.json \
  --vault-dir "~/Documents/Obsidian Vault/01_Literatures/21_Research_Knowledge_Graph"
```

Research mode generates paper and curated concept notes only. Raw Zotero subcollections stay in paper frontmatter as provenance (`zotero_collections_raw`) and do not become wikilink nodes by default.
