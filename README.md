...
- Writes a browser-friendly `START_HERE.html` onboarding guide alongside `START_HERE.md` with recommended next actions, a source setup matrix, and current readiness hints for Gmail, Mail.app, mbox, BibTeX/RIS, web metadata, RSS/Atom, and arXiv, including custom paths and direct web URLs from `reader.env`.
...
The generated `START_HERE.html` includes recommended next actions and a source setup matrix with local readiness hints. Pick one source, follow its prepare/check/run commands, and run a live source check before scheduling automation.
