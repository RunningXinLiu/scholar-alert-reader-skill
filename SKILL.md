...
1. For a new local setup, prefer `quickstart` to create a project, run private-data-free checks/demos, write `QUICKSTART_REPORT.md`, and provide `START_HERE.html` / `START_HERE.md` as the first local onboarding screen. Use `init-project` when the user wants only the scaffold. The CLI can be invoked as `python3 -m scholar_alert_reader`, an installed `scholar-alert-reader` / `scholar-reader` command, or the compatibility wrapper `python3 scripts/scholar_reader.py`.
...
- `START_HERE.md` and `START_HERE.html`: local onboarding guide with first-run workflow, source choices, optional integrations, and setup status.
